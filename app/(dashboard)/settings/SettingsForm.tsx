...
function PlanBadge({ plan }: { plan: string }) {
  ...
  return (
    <span
      ...
      style={{
        fontSize: 12,
        color: planColors[plan] ?? "#555250",
        background: `${planColors[plan] ?? "#555250"}15`,
        border: `0.5px solid ${planColors[plan] ?? "#555250"}30`,
        fontFamily: "var(--font-inter)",
        fontWeight: 500,
      }}
    >
...
export function SettingsForm({ initialData, plan }: SettingsFormProps) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-10 max-w-xl">
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <Field label={ctaLabels[form.ctaType] ?? "CTA value"}>
            <TextInput
              value={form.ctaValue}
              onChange={(v) => update("ctaValue", v)}
              placeholder={ctaPlaceholders[form.ctaType] ?? ""}
            />
          </Field>
        </div>
      </section>
      ...
      {/* Enterprise section (always visible as a teaser) */}
      <section
        style={{
          background: "#0F0E0B",
          border: "0.5px solid #1E1C18",
      ...
      <div className="flex items-center gap-3">
        ...
        {saved && (
          <span style={{ fontSize: 12, color: "#4CAF50", fontFamily: "var(--font-inter)" }}>
            ✓ Saved
          </span>
        )}
      </div>
